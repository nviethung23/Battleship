...
function renderPlacementBoard() {
    ...
    // CRITICAL: Clear ALL contents to prevent duplicates
    board.innerHTML = '';
    
    // console.log('[Placement] 🚢 Rendering board with', gameState.myShips.length, 'ships');

    // Render all cells - ships are shown as BLACK FILLED CELLS (no images)
    for (let row = 0; row < GRID_SIZE; row++) {
        for (let col = 0; col < GRID_SIZE; col++) {
            ...
            if (shipAtCell) {
                ...
                // Make ship cells DRAGGABLE to reposition
                cell.draggable = true;
    ...
    // Add keyboard listener for R key rotation on board
    setupBoardRotationListener();
    
    // console.log('[Placement] ✅ Rendered board with ships as filled cells (no images)');
}
...
function renderBattleMyBoard() {
    console.log('[Game] 🎨 Rendering my battle board...');
    ...
    // Use gameState.myBoard which has the ships
    for (let r = 0; r < GRID_SIZE; r++) {
        for (let c = 0; c < GRID_SIZE; c++) {
            ...
            // Check if this cell has a ship from gameState
    ...
    console.log('[Game] ✅ My battle board rendered');
}

function renderBattleEnemyBoard() {
    console.log('[Game] 🎨 Rendering enemy battle board...');
    ...
    console.log('[Game] ✅ Enemy battle board rendered');
}
